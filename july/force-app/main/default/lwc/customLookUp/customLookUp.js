...
export default class CustomLookUp extends LightningElement {
    ...
    @wire(getRecord, { recordId: '$recordId', fields: ['asset.TKD_rb_collateral__c','asset.TKD_rb_collateral__r.COLATERAL_DESCRIPTION__c','asset.TKD_rb_collateral__r.COLATERAL_TYPE__c'] })
    async wireRecord({ error, data }) {
        if(data){
            this.assetRecord = data
            console.log('data searched ' + JSON.stringify(data))
            if(data.fields.TKD_rb_collateral__r.value !== null & data.fields.TKD_rb_collateral__r.value !== '' & data.fields.TKD_rb_collateral__r.value !== undefined){
                this.collateralDescription = data.fields.TKD_rb_collateral__r.value.fields.COLATERAL_DESCRIPTION__c.value
                this.collateralType = data.fields.TKD_rb_collateral__r.value.fields.COLATERAL_TYPE__c.value
                this.initialSelection =[
                    { id: this.recordId, sObjectType: '', icon: 'custom:custom44', title: data.fields.TKD_rb_collateral__r.value.fields.COLATERAL_DESCRIPTION__c.value, subtitle: data.fields.TKD_rb_collateral__r.value.fields.COLATERAL_TYPE__c.value }
                    ]
