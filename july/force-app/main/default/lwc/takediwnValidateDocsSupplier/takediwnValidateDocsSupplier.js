...
export default class TakediwnValidateDocsSupplier extends LightningElement {
  @api recordId;
  @api docshowModal;
  @track url;
  @track urlContent;
  ...
  connectedCallback() {
    registerListener("validasup", this.validacionremote, this);
  }
  ...
  async openUrl() {
    ...
    let messageStructure = messageStructureArr.map((val, index) => {
      if(val.TKD_tx_Vat_Registration_Num__c){
        return {
          //identifier: val.Id,
          name: val.Name,
          rfc: val.TKD_tx_Vat_Registration_Num__c.replace(/&/g, '_'),
          type: 1, //"SUPPILERS",
  ...
  async validateSuppilerDocs(encriptedData) {
    ...
    if ((tolgate.identifiers !== undefined) & (tolgate.identifiers !== null)) {
      ...
      if (requiredVilations.length === 0 && this.suppilersDocuments === false) {
        ...
        validateSuppDocsts({ recordId: this.recordId, validation: true })
          .then(res => {
            console.log(
              "Se actualizo correctamente la validacion de suppliers"
            );
          })
          .catch(errupdate => {
            console.log("Fallo la validacion de suppliers");
            console.log(errupdate);
          });
      } else if (
        ...
        validateSuppDocsts({ recordId: this.recordId, validation: false })
          .then(res => {
            console.log(
              "Se actualizo correctamente la validacion de suppliers"
            );
  ...
  validacionremote(mensaje){
      console.log('Se dispara la validacion remota ' + mensaje );
      this.openUrl();
  }
}