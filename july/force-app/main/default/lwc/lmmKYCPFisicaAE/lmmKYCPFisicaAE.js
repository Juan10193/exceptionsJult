...
import LMM_ls_Si_es_casado from '@salesforce/schema/LMM_KYC__c.LMM_ls_Si_es_casado__c'
import LMM_tx_Dependientes_Econ_micos from '@salesforce/schema/LMM_KYC__c.LMM_tx_Dependientes_Econ_micos__c'
...
export default class LmmKYCPFisicaAE extends LightningElement {
    @api recordId
    @api title;
    @api message;
    @api variant;
    ...
    @track regimenSC = false
    @track regimenBS = false
    ...
    async wireRecord(result) {
        ...
        if (result.data) {
            ...
            if (this.kycRecord.LMM_ls_Si_es_casado__c.value === 'Régimen Sociedad Conyugal') {
                this.regimenSC = true
                this.regimenBS = false
            } else if (this.kycRecord.LMM_ls_Si_es_casado__c.value === 'Régimen Bienes Separados') {
                this.regimenSC = false
                this.regimenBS = true
            }
    ...
    handleSave() {
        ...
        if(this.template.querySelector("[data-field='regimenSC']").checked === true){
            fields[LMM_ls_Si_es_casado.fieldApiName] = 'Régimen Sociedad Conyugal'
        } else if(this.template.querySelector("[data-field='regimenBS']").checked === true){
            fields[LMM_ls_Si_es_casado.fieldApiName] = 'Régimen Bienes Separados'
        }
        fields[LMM_tx_Dependientes_Econ_micos.fieldApiName] = this.template.querySelector("[data-field='nacionalidad']").value
